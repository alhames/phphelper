...
class Image
{
    ...
    protected function __construct(?string $data = null, ?string $file = null)
    {
        ...
        $hash = $isFile ? Hash::createFromMd5File($file) : Hash::createFromMd5($data);
        $this->name = $hash->getUriSafeBase64();
     ...
     */
    public function save(string $directory, ?string $name = null): void
    {
        try {
            if (!is_dir($directory)) {
     ...
     * @param string $path
     */
    protected function saveAsFile(string $path): void
    {
        if (null !== $this->file) {
            copy($this->file, $path);
        } else {
